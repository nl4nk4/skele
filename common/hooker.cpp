...
{
    // wait until the game has loaded before we contiue
    do
    {
...
{
    skele_print("injected");
    
    // todo : update on resize, and recreate fonts
    g_engine->get_screen_size(set.screen.w, set.screen.h);
    
    // bug
    // you need to resize the mutli combos or theyll crash
    set.visuals.player.example.resize(3);
}
